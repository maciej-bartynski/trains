...
class MenuTrainSetRoute extends StatefullComponent<ElementState, ElementProps> {
        ...
        if (!params.resourceKind) {
            return `
                <li class="${classNames.header.cargo.listItem} list_item">
                    <span class="${classNames.header.cargo.listItemLabel}">0/10</span>
                </li>
            `;
        }
    ...
    constructor() {
        ...
        this.renderCargoSection = this.renderCargoSection.bind(this);
        this.renderTrainShort = this.renderTrainShort.bind(this);
    ...
    renderTrainShort(props: {
        ...
        const destinationField = destinationEvent?.state.address
            ? GameBoard.getInstance().getField(destinationEvent.state.address)
            : null;
        const destinationBuilding = destinationField
            ? destinationField.state.building
            : null;
        const buildingLabel = destinationBuilding
            ? BuildingUtils.BuildingKindToDisplayName[destinationBuilding]
            : 'Destination'
        const destinationLabel = destinationEvent
            ? `${buildingLabel} at C:${destinationEvent?.state.address.column ?? '(?)'} &#10005; R:${destinationEvent?.state.address.row ?? '(?)'}`
            : 'Destination';

        const eventStateToLabelMap = {
            departure: `Departuring to ${destinationLabel}`,
            arrival: `Arriving to ${destinationLabel}`,
            trespassing: `Heading to ${destinationLabel}`,
            awaiting: 'Awaiting...',
            ready: 'Train ready',
            dump: 'Unloading cargo...',
            'pick-up': 'Loading cargo...',
        }
        ...
        const locationGridLabel = `<span class="${classNames.locationGridLabel}">${getLocationGridLabel(field.state.address)}</span>`
        ...
        this.trainStateEl.innerHTML = train.state.operationState
            ? `<span class="indicator-blue" style="display: inline-block"></span>&nbsp;${eventStateToLabelMap[train.state.operationState]}`
    ...
    renderCargoSection(props: {
        train: TrainModel
    }) {
        const { train } = props;

        const cargoList: ([ResourceKind, number] | [null, null])[] = [];

        Object.entries(train.state.cargo ?? {}).forEach(cargo => {
            const [resourceKind, qty] = cargo as [ResourceKind, number];

            if (qty) {
                let cargoLeft = qty;
                while (cargoLeft > 0) {
                    const cargoToLoad = cargoLeft >= TrainModel.maxSlotLoad
                        ? TrainModel.maxSlotLoad
                        : cargoLeft;
                    cargoLeft -= cargoToLoad;
                    cargoList.push([resourceKind, cargoToLoad]);
                }
            }
        });

        if (cargoList.length < TrainModel.cargoSlots) {
            const placeholderSlotsAmount = TrainModel.cargoSlots - cargoList.length;
            for (let i = 0; i < placeholderSlotsAmount; i++) {
                cargoList.push([null, null])
            }
        }

        this.cargoList.innerHTML = cargoList.map(cargo => {
            const [resourceKind, qty] = cargo as [ResourceKind, number] | [null, null];
            if (resourceKind) {
    ...
    override render() {
        ...
        if (train && currentField) {
            ...
            this.renderCargoSection({ train })

            this.desinationsList.innerHTML = '';
            const routesToRender = train.state.originalJurney;

            const firstDepartureEvent = train.state.originalJurney?.[0]?.[0] ?? null;

            let firstDepartureField = firstDepartureEvent
                ? GameBoard.getInstance().getField(firstDepartureEvent.state.address)
                : currentField;

            if (firstDepartureField) {
                const firstRouteItem = RouteItem.createRouteItem();
                firstRouteItem.setProps({
                    route: [],
                    routeIndex: 0,
                    journey: routesToRender,
                    train: train,
                    journeyDepartureStation: firstDepartureField
                });
                this.desinationsList.appendChild(firstRouteItem);
            }

            routesToRender.forEach((route, routeIndex, journey) => {
                const routeItem = RouteItem.createRouteItem();
                routeItem.setProps({
                    route,
                    routeIndex,
                    journey,
                    train: train,
                    journeyDepartureStation: null
                });
                this.desinationsList.appendChild(routeItem);
            });

            if (train.state.originalJurney.length) {
                this.contentSection.appendChild(this.goCta);
                this.goCta.onclick = () => {
                    const firstRoute = train.state.originalJurney[0];
                    const firstDestination = firstRoute
                        ? firstRoute[firstRoute.length - 1]?.state.address
                        : null;
                    if (firstDestination && train.state.originalJurney.length) {
                        train.setJourney({
                            journey: train.state.originalJurney
                        })
                    }
                }
            } else {
...
const innerHtmlText = `
    <div class="box-secondary card-primary">
        ...
        <section class="${classNames.content}">
            ...
            <div class="list_header ${classNames.destinations.header}">
                Journey
            </div>
